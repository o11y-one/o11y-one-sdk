...
OUTPUTS=(
  ...
  # Per-domain agentic subset (agentic + common only). Regenerated by the
  # scoped `buf generate` pass below into the packages the agentic SDKs depend
  # on. Listed here so a failed generate restores them too and a domain removed
  # from the closure disappears from the subset as well. See
...
echo "==> buf generate (whole tree: all domains -> gen/go, gen-ts, gen-py)"
run_buf || exit 1

echo "==> buf generate (agentic subset: agentic + common -> gen-ts-agentic, gen-py-agentic)"
