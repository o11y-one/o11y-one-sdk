...
# (useful when only buf.gen.yaml changed). Generated code is COMMITTED; this
...
#      a full run is six requests, which is normally fine, but a busy shared
...
# survive (gen/go/go.mod, the two package manifests), which is why buf.gen.yaml
# sets `clean: false` and the wiping happens here, scoped to these paths.
OUTPUTS=(
  gen/go/o11y_one
  packages/gen-ts/src/o11y_one
  packages/gen-py/src/o11y_one
)
...
if [[ -f packages/gen-py/src/o11y_one/__init__.py ]]; then
  echo "packages/gen-py/src/o11y_one/__init__.py exists — that would turn the" >&2
