...
# survive (the go.mod files, the package manifests), which is why buf.gen.yaml
# sets `clean: false` and the wiping happens here, scoped to these paths.
OUTPUTS=(
  # The whole tree, every domain. Never published (docs/proto-subsetting.md).
  internal/gen/go/o11y_one
  packages/gen-ts/src/o11y_one
  packages/gen-py/src/o11y_one
  # The agentic closure (agentic + common only): the published surface.
  # Regenerated by the scoped `buf generate` pass below. Listed here so a
  # failed generate restores them too and a domain removed from the closure
  # disappears from the subset as well. See docs/proto-subsetting.md.
  gen/go/o11y_one
...
# a real signal. Used for both the whole-tree pass and the scoped agentic pass.
run_buf() {
...
echo "==> buf generate (whole tree: all domains -> internal/gen/go, gen-ts, gen-py)"
run_buf || exit 1

echo "==> buf generate (agentic subset: agentic + common -> gen/go, gen-ts-agentic, gen-py-agentic)"
...
for py_ns in packages/gen-py/src/o11y_one packages/gen-py-agentic/src/o11y_one; do
